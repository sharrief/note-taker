 ...
 */
interface BusyButtonRequiredProps {
  text: string;
  ariaLabel: string;
  disabled: boolean;
  onClick: () => void;
 ...
 */
interface BusyButtonDefaultProps {
  variant: ''|'primary'|'secondary'|'info'|'success'|'warning'|'error';
  busy: boolean;
}

/** All props for the BusyButton */
type BusyButtonProps = BusyButtonRequiredProps & BusyButtonDefaultProps

/**
 * Renders a clickable button with optional styling and busy state
 * @param {BusyButtonProps} props The props for the button
 */
export default function BusyButton({ text, ariaLabel, disabled, onClick, busy = false, variant = '' }: BusyButtonProps
) {
  return <button
    type="button"
